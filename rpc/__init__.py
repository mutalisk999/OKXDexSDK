...
async def check_nonce(_node_url: str, _caller_addr: str) -> Optional[int]:
    w3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(_node_url))
    connected = await w3.is_connected()
    if not connected:
        return None

    nonce = await w3.eth.get_transaction_count(AsyncWeb3.to_checksum_address(_caller_addr))
    return nonce
...
if __name__ == "__main__":
    ...
    async def func():
        ...
        r1, r2, r3 = await asyncio.gather(fut1, fut2, fut3)
        print(r1)
        print(r2)
        print(r3)
